        ...
        <div className="w-full max-w-[24rem]">
          ...
          <div className="space-y-2">
            { HackClubContent.map((link, idx) => (
              <a key={idx} href={link.href} className="text-sm block text-zinc-300 transition hover:underline hover:text-amber-400">{link.text}</a>
            )) }
          </div>
        </div>
        <div className="w-full max-w-[24rem]">
          ...
          <div className="space-y-2">
            { Resources.map((link, idx) => (
              <div className="block" key={idx}>
                <a href={link.href} className="text-sm text-zinc-300 transition hover:underline hover:text-amber-400">{link.text}</a>
              </div>
            )) }
          </div>
        </div>
